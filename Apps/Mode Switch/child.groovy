 ...
 *  V1.0.2 - Debug light levels
 *  V1.0.1 - Revised update check
 *  V1.0.0 - POC
 *
 */
...
definition(
    ...
    namespace: "Cobra",
    author: "AJ Parker",
    description: "This is designed to use location modes to control switches/lights etc",
    category: "My Apps",
...
def mainPage() {
    dynamicPage(name: "mainPage") {  
     ...
     section() {input "newMode1", "mode", title: "Which Mode(s) do you want to react to?", required: true, multiple: true}      
	 section(){
     ...
     if(light1){ input (name: "dimLevel1", type: "number", title: "To this level (set to 0 for off)", required: true)}
	...
	if(light2){input (name: "dimLevel2", type: "number", title: "To this level (set to 0 for off)", required: true)}
	     }
    }
}
...
def runNow(){
    ...
    if(light1){state.dim1 = dimLevel1.toInteger()}
    if(light2){state.dim2 = dimLevel2.toInteger()}
...
def setVersion(){
		...
		state.preCheckMessage = "This is designed to use location modes to control switches/lights etc."
