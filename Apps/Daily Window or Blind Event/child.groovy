 ...
 *  V1.0.1 - Revised update checking
 *  V1.0.0 - POC
 *
 *
 *  Sets a schedule to open a window/vent/blind to a set level, then schedule it to close again 
 *
...
definition(
    name: "Daily Window or Blind Event Child",
    namespace: "Cobra",
    author: "Andrew Parker",
    description: "Schedule a window/vent/blind to open to a set level then automatically close when you specify.",
    category: "Convenience",
    
   parent: "Cobra:Daily Window or Blind Event",
...
def mainPage() {
	...
	section("Open time...") {
		input (name: "switchOnTime", title: "At what time?", type: "time",  required: false)
	}
    section("Close time...") {
		input (name: "switchOffTime", title: "At what time?", type: "time",  required: false)
	}
...
def subscribeNow() {
  ...
  // App Specific subscriptions & settings below here
    ...
    if(switchOnTime){schedule(switchOnTime, switchOnNow)}
    if(switchOffTime){schedule(switchOffTime, switchOffNow)}
}
...
def switchOnNow(evt) {
	...
	LOGDEBUG("Opening $theSwitch...")
    theSwitch.setLevel(myLevel)
    LOGDEBUG("$theSwitch is set to $myLevel")
	} 
 }
...
def switchOffNow(evt){
    checkAllow()
	if(state.allAllow == true){
	LOGDEBUG("Switching back off")
	theSwitch.off()
	LOGDEBUG("$theSwitch is now off")
    }
}
...
def setVersion(){
		...
		state.preCheckMessage = "Sets a schedule to open a window/vent/blind to a set level, then schedule it to close again " 
