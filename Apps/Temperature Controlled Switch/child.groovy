 ...
 *
 *
 *  V1.6.0 - Added additonal switch for restriction 
...
if (state.contact1Now == 'open'){
	LOGDEBUG("$contact1 is $state.contact1Now - Switching off now...")
switch2.off()
	LOGDEBUG("$switch2 is OFF - Heating/Cooling Disabled")
	}
 else{
LOGDEBUG("Contact is $state.contact1Now - Heating/Cooling NOT restricted by $contact1")
  }	
...
LOGDEBUG("Configured for cooling mode")
	...
	if(state.allAllow == false){switch2.off()}
