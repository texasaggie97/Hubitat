...
preferences {
 ...
 def mainPage() {
	...
	preCheck()
      section() {  
      input "trigger", "enum", title: "Select Trigger", submitOnChange: true, options: [ "Time","Switch On", "Contact Open","Button Pushed"]   
      } 
        ...
        if(trigger){     
			...
			if(trigger == 'Contact Open'){section() {input "triggerContact", "capability.contactSensor", title: "Select Contact", multiple: true}}
