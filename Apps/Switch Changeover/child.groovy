 ...
 *
 *
 *  V2.0.0 - Added additonal (2nd) switch for restriction & fixed other restriction bugs
 *  V1.9.1 - Debug presence restriction
 *  V1.9.0 - Added disable apps code
 *  V1.8.0 - Streamlined restrictions page to action faster if specific restrictions not used.
 *  V1.7.0 - Moved update notification to parent
 *  V1.6.0 - Added 'restrictions' page & code cleanup
 *  V1.5.0 - Made parent/child and added to 'Cobra Apps'
 *  V1.4.0 - Ported to Hubitat and setup update checking
...
definition(
    name: "Switch Changeover Child",
    namespace: "Cobra",
    author: "Andrew Parker",
    description: "If you turn on a switch - All others turn off. <br>Two switches cannot be on at the same time.",
    category: "Convenience",
 
    parent: "Cobra:Switch Changeover",
...
def subscribeNow() {
  ...
  // App Specific subscriptions & settings below here   
    ...
    if(switch1){subscribe(switch1, "switch", switchHandler1)}
    if(switch2){subscribe(switch2, "switch", switchHandler2)}
    if(switch3){subscribe(switch3, "switch", switchHandler3)}
    if(switch4){subscribe(switch4, "switch", switchHandler4)}
    if(switch5){subscribe(switch5, "switch", switchHandler5)}
    if(switch6){subscribe(switch6, "switch", switchHandler6)}
}
...
def setVersion(){
     state.version = "2.0.0"
     state.InternalName = "SwitchChangeOverChild"
	state.ExternalName = "Switch Change Over Child"
	state.preCheckMessage = "If you turn on a switch - All others turn off. <br>Two switches cannot be on at the same time." 
	state.CobraAppCheck = "switchchangeover.json"
