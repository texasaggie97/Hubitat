 ...
 *
 *  Design Usage:
 *  This app is designed to announce via PushOver if any contacts are left open when leaving 
 *
 *
 ...
 *
 *
 *  V1.1.0 - Added additonal (2nd) switch for restriction & fixed other restriction bugs
 *  V1.0.1 - Debug presence restriction
 *  V1.0.0 - POC
 *
 */
...
definition(
    name: "Did I leave Something Open Child",
    namespace: "Cobra",
    author: "Andrew Parker",
    description: "This was designed to announce if any contacts are open when you leave",
    category: "",
    
 parent: "Cobra:Did I leave Something Open",
...
def mainPage() {
	...
	preCheck()
    section(){
	input "leavingPresence",  "capability.presenceSensor", title: "Select presence sensor(s) to monitor", required: true, multiple: true  
	input "sensors", "capability.contactSensor", title: "Contact Sensors to check when left", required: true, multiple: true
	input "speaker1", "capability.speechSynthesis", title: "Pushover Device", required: true, multiple: true
	}
	section() {
	input "message1", "text", title: "Message to send before list of open devices",  defaultValue: "Look, I know that you are just leaving BUT.. You left following the windows or doors open:", required: true
	input "delay1", "number", title: "Seconds delay after leaving before sending message (Set to 0 for no delay)",required: true, defaultValue: 0
//	input "priority1", "enum", title: "PushOver Message Priority",required: true, submitOnChange: true, options: ["None", "Low", "Normal", "High"], defaultValue: "None"
	}
  }
}
...
def subscribeNow() {
  ...
  // App Specific subscriptions & settings below here   
	...
	subscribe(leavingPresence, "presence", evtHandler)
	state.timer = 'yes'
}


def evtHandler(evt){
    LOGDEBUG("Running evtHandler... Event received: $leavingPresence $evt.value") 
	state.ps1 = evt.value
	if(state.ps1 == "not present"){
    checkAllow()
	if(state.allAllow == true){
		def	msgDelay = delay1
	LOGDEBUG("$leavingPresence just left so I'll send a message in $msgDelay seconds if any configured contacts are open") 
	
		if(msgDelay == null){msgDelay = 0}
		runIn(msgDelay,talkNow1)
	}
	}
	else{LOGDEBUG("$leavingPresence just arrived..") }
...
def talkNow1() {
	...
	LOGDEBUG("Pushover device in use: $speaker1")     
	def open = sensors.findAll { it?.latestValue("contact") == 'open' }
		if (open) { 
		LOGDEBUG("Open windows or doors: ${open.join(',')}")
		state.fullMsg1 = "$newmsg ${open.join(',')}"
		LOGDEBUG("Sending message: '$state.fullMsg1' to: $leavingPresence")	
		speaker1.speak(state.fullMsg1)
		}
		if(!open) { 
		LOGDEBUG("You remembered to close everything!")
      }
...
def setVersion(){
		state.version = "1.1.0"	 
		state.InternalName = "LeftOpenChild"
    	state.ExternalName = "Did I leave Something Open Child"
		state.preCheckMessage = "This app is designed to send a PushOver message if any contacts are left open when leaving "
    	state.CobraAppCheck = "leftopen.json"
}
