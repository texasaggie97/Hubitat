 ...
 *
 *  
 *  Based upon work done by Steve Bratt
 *
 ...
 *
 *  Note: Within the Device settings the Device Network ID for device must be hex of IP address and port
 *  in the form of 00000000:0000 (i.e. 10.0.1.220:80 is 0A0001DC:0050) - if you check the logs the device will log the 
 ...
 *
 *
 *  V1.0.0 - POC - Ported from ST
 */
 
metadata {
  definition (name: "Sony Bravia TV", namespace: "Cobra", author: " ") {
    capability "Switch"
 //   capability "Polling"
   capability "Refresh"
      
      attribute "HDMI" , "string"
...
//    command "Social"
  ...
  preferences {
		...
		input name: "ipadd1", type: "", required: true, title: "TV IP address", displayDuringSetup: true
...
def updated(){
	...
	log.debug( "Preferences Updated rebuilding MAC address and Hex Network ID")
...
def WOLC() {
	log.info "WOL activated"
    sendHubCommand(createWOL())
}

def createWOL(evt) {
    log.debug "Sending Magic Packet to: ${state.tv_mac}"
    def result = new hubitat.device.HubAction (
...
//   UPDATE CODE NOT NEEDED BELOW HERE.....................
...
def setVersion(){
		...
		state.InternalName = "SonyTV"   // this is the name used in the JSON file for this driver
}
