metadata {
	definition (name: "Aeon Multisensor 6", namespace: "Cobra", author: "AJP"){    // , runLocally: true, minHubCoreVersion: '000.020.00008', executeCommandsLocally: true) {
...
def updated() {
	...
	LOGDEBUG( "${device.displayName} is now ${device.latestValue("powerSource")}")
...
def parse(String description) {
	...
	LOGDEBUG( "After zwaveEvent(cmd) >> Parsed '${description}' to ${result.inspect()}")
	return result
}
...
def ping() {
	if (device.latestValue("powerSource") == "dc") {
		...
		LOGDEBUG( "Can't ping a wakeup device on battery")
	}
}

def configure() {
	// This sensor joins as a secure device if you double-click the button to include it
	LOGDEBUG( "${device.displayName} is configuring its settings")
	def request = []
...
def LOGDEBUG(txt){
    try {
    	if (settings.debugMode) {log.debug "Aeon Multi 6 Driver - ${txt}" }
    } catch(ex) {
